...
func load_scene(scene_name):
	if scene_name in Global.choices:  # Verifica se a cena existe no dicionário Global
		current_scene = scene_name
		var scene_data = Global.choices[scene_name]
		
		# Atualiza o texto da Label com a nova situação
		$Label.text = scene_data["texto"]
...
func _on_choice_selected(choice):
	...
	load_scene(next_scene)  # Muda a cena para a nova escolha

# Função que roda ao iniciar a cena
func _ready():
	load_scene(current_scene)  # Começa na primeira cena definida
